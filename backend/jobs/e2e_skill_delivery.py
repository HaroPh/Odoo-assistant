...
"""Job e2e-skill-delivery — wrap live_verify_skill_delivery thành job
...
MCP_PORT = 8001
SCRIPT = REPO_ROOT / "backend" / "tests" / "live_verify_skill_delivery.py"
ODOO_NOTE = "tạo/xác nhận SO + giao kho THẬT trong Odoo (delivery) — dọn tay nếu cần"
...
def run(args) -> JobResult:
    ...
    if err:
        ...
        return JobResult("e2e-skill-delivery", INFRA_ERROR, "ERROR", {"preflight": err})
    ...
    try:
        ...
        return JobResult("e2e-skill-delivery", INFRA_ERROR, "ERROR", detail)
    ...
    if result_json["passed"] == result_json["n"]:
        return JobResult("e2e-skill-delivery", PASS, "PASS", detail)
    return JobResult("e2e-skill-delivery", GATE_FAIL, "FAIL", detail)


register(Job("e2e-skill-delivery", run,
             "E2E skill agentic: delivery (3 kịch bản, cần full stack + write thật)",
             schedulable=False))
