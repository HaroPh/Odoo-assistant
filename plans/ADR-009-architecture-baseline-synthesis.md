...
| S2 | **Chuẩn retry cho Job Runner:** bounded + checkpoint + circuit-breaker, 1 helper dùng chung | F15 |
| S3 | **Toggle vận hành qua cơ chế Odoo-native** (WRITE_ACTIONS_ENABLED…) — không redeploy; secret ở secret-store đúng | F16 |
...
🔴 Phase 0 (Hardening-Verify) — MỚI, cổng bắt buộc, effort thấp (verify không build)
   ...
   Phase A (AI Layer hybrid)
     A1 alias gateway (0.5d) → A2 ROLE_MODELS + /no_think điều kiện (1–2d, gồm M4/S4)
        ...
        → A3 flip 3/7 role (Router/Eval/Chit-chat) sang cloud;
          Read/Fusion/Synthesis ở LOCAL (QĐ M2); gồm M5 trim history chit-chat
...
| R7 | **thread_id không scope theo user** (M1b): collision same-first-message (bug ngay single-user); rò quyền-ghi chéo nếu >1 user. **Lưu ý demo (review fable):** kịch bản demo bán hàng LẶP LẠI câu mở đầu canned ("tạo báo giá cho Azure Interior...") → xác suất collision trong demo là CAO, không lý thuyết — hai chat cùng câu mở đầu nối chung state/working_context. TTL 300s giới hạn phần confirm treo, không giới hạn phần state chung. | TB | Deployment 1-user; TTL 300s cho confirm treo | Trước mỗi demo: dùng câu mở đầu khác nhau hoặc restart backend. Sửa thật (user identity + nonce per-conversation) TRƯỚC multi-user; cần spike xem Open WebUI có forward được chat-id không (fallback sha1 tồn tại vì Open WebUI không gửi session_id) |
...
| R5 | **Eval-harness chưa tự động** — M3 hiện là kỷ luật (276 test + 3 smoke thủ công) | TB | Roadmap đẩy eval-job lên sớm (Phase C song song A2) | Tới khi eval-job tồn tại, "gate trước flip" phụ thuộc kỷ luật con người — chính là bài học F10 |
| R6 | **Quota Guard chưa build (A4)** | Thấp→**TB** (xem R8) | Degrade-to-local tự động khi cloud lỗi/hết quota (hệ không chết) | Không cảnh báo chủ động 80%/95%; âm thầm chậm tới khi A4 xong |
| R8 | **MỚI (2026-07-09, đo thật lúc Task 6):** RPD free-tier THẤP HƠN NHIỀU giả định gốc của spec §0. Số đo thật qua Google AI Studio console: `gemini-3.1-flash-lite` = **36 RPD** (spec giả định 500/ngày cho Router — thấp hơn ~14×); `gemma-4-26b` = **2 RPD** (gần như không dùng được — 1 phiên chat có thể ăn hết). Eval-gate run (64 call cho gemini-flash-lite) ĐÃ chạm rate limit ngay trong 1 lần chạy. | Cao (thực tế vận hành) | LiteLLM fallback khai báo (`gemini-flash-lite→qwen3:8b`) đã hoạt động đúng thiết kế — hệ tự rơi về local khi hết quota, không lỗi. A4 (Quota Guard) hiện HẠ ưu tiên trong roadmap vì lý do "500-1500 RPD khó chạm 1 user" — **lý do đó SAI với số đo thật, cần re-evaluate lại thứ tự ưu tiên A4** khi lập plan tiếp theo. Google đã báo "set up billing to increase limits". | Router flip (Task 6) chỉ có giá trị THỰC TẾ trong ~36 request/ngày đầu, sau đó âm thầm chạy local phần còn lại — chấp nhận được (không lỗi) nhưng lợi ích cloud không đều/không dự đoán được như kỳ vọng ban đầu. `gemma-cloud` (chit-chat) coi như KHÔNG khả dụng ở free-tier hiện tại (2 RPD) — quyết định Task 6: KHÔNG flip chitchat. |
